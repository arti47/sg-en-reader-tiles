...
- [ ] T18 advanced decode ladder (open syllables, C-le, prefixes/suffixes/morphology, schwa) *(**inflectional-suffix slice authored 2026-07-21** — `PH-suffixes`/`SP-suffixes` (-s/-ing/-ed with add/double/drop-e/change-y) via `scripts/gen-suffixes.mjs`, 41 decode + 41 encode + lessons, lint-clean, wired + smoke-green, gated behind two-syllable. **Box stays open** pending: (a) maxing to the §6d ceiling (~60), (b) the rest of the tier — open syllables, C-le, prefixes, schwa — still unauthored, and (c) owner §12.5 sign-off. Do not tick until signed off.)*
