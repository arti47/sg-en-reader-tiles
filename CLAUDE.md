...
**M4 — Polish. ◑ CORE MET (2026-07-20).** Self-hosted dyslexia font (Lexend default + OpenDyslexic opt-in, `data-font`); parent Settings (font, voice speed, session length) persisted + applied; gamification (`gamify.ts` — XP/level badge on picker, "+N XP" on summary); background cache pre-warm on `visibilitychange`; a11y (labelled steppers, aria-live). **Still open:** threshold tuning against real use (needs the family's live data), deeper gamification (achievements/daily quests), full accessibility audit + `ui.ts` shared modal/toast primitives.
...
- [ ] T14 grammar starter track (articles, SVA-simple, tenses-basic) *(authored 2026-07-20 — `grammar-L01-articles` + `grammar-L02-sva`, 10 items each + lessons, lint-clean; tenses-basic + owner sign-off pending)*
- [ ] T15 vocab starter track *(authored 2026-07-20 — `vocab-L01-synonyms`, 10 items + lesson, lint-clean; more sets + sign-off pending)*
- [ ] T16 PSLE Paper-2 comprehension + cloze *(authored 2026-07-20 — `comp-L01` (passage_question) + `cloze-L01-grammar` (grammar_cloze word-bank), 10 items each + lessons, lint-clean; expand + sign-off pending)*
