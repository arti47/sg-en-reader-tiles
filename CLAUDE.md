...
- [ ] T12 high-frequency word sets (threaded) *(first set authored via `scripts/gen-hf.mjs` + wired + lint-clean 2026-07-20, **55 recognition items** + lesson; uses §6a highFrequency escape hatch; pending owner sign-off. NOTE: currently gated after T10 (prereq PH-two-syllable) to avoid session-timing churn — true "threading" through every session needs SRS/session-composition work, deferred)*
