...
**Dual mastery gates (phonics patterns).** A pattern is mastered only when BOTH its decode skill and its encode (spelling) skill pass. Encode unlocks after decode reaches ~70% (guided overlap), but advancement requires both. `minItems` for foundational decode/encode is **12** (overlearning for struggling readers; 85%×12 ≈ 11/12).
...
**Spaced repetition (replaces vague "review %").** On acquisition, schedule reviews at **+2d, +7d, +21d**. Session composition: due reviews first (cap ~4/session), then current-skill items. The **first review pass mints the withheld certificate**; a failed review demotes the skill (short re-practice block, easier items) until re-passed. Post-M4: tune intervals.
...
**Placement (dual — critical for this audience).** Capture **chronological P-level** at add-time (sets PSLE-component expectations), then run an **independent reading placement** that finds true instructional level per strand by walking **down** on failure / **up** on success. For struggling readers these diverge — place content by **reading level, not P-level**. Start decoding placement low and confirm foundations before advancing. Keep placement short (≤ ~15 items) and low-pressure: **frame it as a warm-up game, show no right/wrong feedback during placement**, end on an achievable item. **Spelling is NOT free-placed (2026-07-21):** the warm-up only tests decoding, so a high placement credits the lower rungs' decode+encode but **holds back the TOP read rung's spelling** — the child must earn it in-session before the decode entry unlocks (`encodeUnlocked` treats a placement-confirmed decoder as ≥70%). Spelling lags decoding in dyslexia.
