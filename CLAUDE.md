...
- [ ] T11 spelling packs mirroring T01–T10 (encode items + dictation sentences) *(encode packs L02–L10 owner-approved; **dictation now built** — new `dictation` item type + `DictationItem` renderer + `scoreDictation` + `dictation-L02-cvc` (20 CVC-sentence items, word-by-word tiles) + `SP-cvc-dictation` skill (gated after two-syllable), lint-clean, smoke-verified. Box stays open: dictation for levels beyond CVC + owner sign-off pending)*
